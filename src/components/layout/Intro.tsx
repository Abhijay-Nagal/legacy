...
export function Intro({ onComplete }: { onComplete: () => void }) {
  ...
  return (
    ...
    >
      <motion.div
        className="inline-flex flex-col items-start px-16 py-8 md:px-24"
        initial={{
          opacity: 0,
          scale: reduce ? 1 : 0.85,
        }}
      ...
      >
        ...
        <div className="flex items-end justify-center">
          <motion.div
            className="-mr-6 shrink-0 md:-mr-12"
          ...
          >
            <LogoMark className="h-44 w-44 md:h-60 md:w-60" />
          </motion.div>

          <SweepText
            text="EGACY"
            className="font-sans text-8xl font-bold tracking-tight md:text-[10rem]"
            reduce={reduce}
            isLight={isLight}
          />
        </div>

        {/* Tagline — left-aligned to start under the glyph's left edge */}
        <SweepText
          text="LEARN. UNDERSTAND. REMEMBER."
          className="mt-4 text-left text-xl font-bold tracking-[0.18em] md:text-4xl"
          reduce={reduce}
          isLight={isLight}
        />
      </motion.div>
